...
batching = false;
		 ...
		 template <int BLOCK_DIM_X = 32, int MAX_ITERATIONS = 400, int CACHE_SIZE = 512, int SORTED_SIZE = 256, bool DIST_STATS = false>
			 void queryLayer() {
				 ...
				 int NUM_ITER = 1;
				 if (!batching) {
				 	MULT /= dataset.N_query;
					NUM_ITER = dataset.N_query;
				 }
				 VLOG(0) << MULT << " " << NUM_ITER << " " << dataset.N_query * MULT << "\n";
				 ...
				 for (int i = 0; i < NUM_ITER; i++) {
					 ...
					 // Loads query to host memory
					 dataset.loadQuery(query_file_path, i, dataset.N_query * MULT);
					 
					 cudaEventRecord(start, shard.stream);

					 // Loads query to GPU memory
					 ggnn_gpu_instance.ggnn_query.loadQueriesAsync(dataset.h_query, shard.stream, MULT);
					 cudaStreamSynchronize(shard.stream);

					 ggnn_gpu_instance.template queryLayer<BLOCK_DIM_X, MAX_ITERATIONS, CACHE_SIZE, SORTED_SIZE, DIST_STATS>(0, MULT);
					 
					 ggnn_gpu_instance.ggnn_query.sortAsync(shard.stream, MULT);
					 ggnn_results.loadAsync(ggnn_gpu_instance.ggnn_query, 0, shard.stream, MULT);
			
					 cudaEventRecord(stop, shard.stream); 
					 cudaEventSynchronize(stop); 

					 cudaEventElapsedTime(&milliseconds, start, stop);
					 VLOG(0) << "[GPU: " << ggnn_gpu_instance.gpu_id << "] query part: " << 0 << " => ms: " << milliseconds << " [" << dataset.N_query * MULT<< " points query -> " << milliseconds*1000.0f/dataset.N_query/MULT << " us/point] \n";

					 cudaEventDestroy(start);				
					 cudaEventDestroy(stop);

					 cudaStreamSynchronize(shard.stream);
					 ggnn_results.merge(MULT);
					 ggnn_results.evaluateResults(i, MULT);	 
				 }
			}
