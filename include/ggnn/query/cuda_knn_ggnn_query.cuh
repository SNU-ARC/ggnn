...
struct GGNNQuery {
	...
	GGNNQuery(const int N_query, const int D, const int K_query, const int num_parts = 1) : N_query{N_query}, D{D}, K_query{K_query}, num_parts{num_parts} {
		CHECK_CUDA(cudaMalloc(&d_query, static_cast<size_t>(N_query) * D * sizeof(BaseT)));
		CHECK_CUDA(cudaMalloc(&d_query_result_ids, static_cast<size_t>(N_query) * K_query * num_parts * sizeof(KeyT)));
		CHECK_CUDA(cudaMalloc(&d_query_result_ids_sorted, static_cast<size_t>(N_query) * K_query * num_parts * sizeof(KeyT)));
		CHECK_CUDA(cudaMalloc(&d_query_result_dists, static_cast<size_t>(N_query) * K_query * num_parts * sizeof(ValueT)));
		CHECK_CUDA(cudaMalloc(&d_query_result_dists_sorted, static_cast<size_t>(N_query) * K_query * num_parts * sizeof(ValueT)));
	...
	void loadQueriesAsync(const BaseT* h_query, const cudaStream_t stream, float MULT) {
		/*
		if (MULT != 1) {
			cudaFree(d_query);
                        cudaFree(d_query_result_ids);
                        cudaFree(d_query_result_ids_sorted);
                        cudaFree(d_query_result_dists);
                       	cudaFree(d_query_result_dists_sorted);

                        CHECK_CUDA(cudaMalloc(&d_query, N_query * D * sizeof(BaseT) * MULT));
                        CHECK_CUDA(cudaMalloc(&d_query_result_ids, N_query * K_query * num_parts * sizeof(KeyT) * MULT));
                        CHECK_CUDA(cudaMalloc(&d_query_result_ids_sorted, N_query * K_query * num_parts * sizeof(KeyT) * MULT));
                        CHECK_CUDA(cudaMalloc(&d_query_result_dists, N_query * K_query * num_parts * sizeof(ValueT) * MULT));
                        CHECK_CUDA(cudaMalloc(&d_query_result_dists_sorted, N_query * K_query * num_parts * sizeof(ValueT) * MULT));
		}
		*/

		size_t SIZE = N_query * D * sizeof(BaseT);

		CHECK_CUDA(cudaMemcpyAsync(d_query,
					h_query,
                                        SIZE * MULT,
                                        cudaMemcpyHostToDevice, stream));
	}

	void sortAsync(const cudaStream_t stream, float MULT) {
		if (num_parts == 1) {
			CHECK_CUDA(cudaMemcpyAsync(d_query_result_ids_sorted, d_query_result_ids,
                                                static_cast<size_t>(N_query * MULT) * K_query * sizeof(KeyT),
                                                cudaMemcpyDeviceToHost, stream));
                        CHECK_CUDA(cudaMemcpyAsync(d_query_result_dists_sorted, d_query_result_dists,
                                                static_cast<size_t>(N_query * MULT) * K_query * sizeof(ValueT),
                                                cudaMemcpyDeviceToHost, stream));
		}
		else {
